...
function addProductToCart(decodedItems, cartObj) {
    try {
        ...
        for (var i = 0; i < productList.length; i++) {
            ...
            // After product data has been decoded & processed using core SFCC logical flows - it is passed to OOTB SiteGen Add To Cart logic (the same logic that handles
            //  'Add to Cart' on the PDP)
            cart.addProductItem(productToAdd.object, productList[i].quantity, productOptionModel);
        }

        // After an item is added to the cart using the addProductItem method on the Cart Model, we return a simple object with expected values that will be used
        //  in SiteGen templates (Ex: minicart template)
