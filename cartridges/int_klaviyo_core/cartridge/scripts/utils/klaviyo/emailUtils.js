...
function prepareOrderPayload(order, isFutureOrder, mailType) {
    var logger = Logger.getLogger(
        'Klaviyo',
        'Core emailUtils - prepareOrderPayload()'
    );
    logger.info('Calling prepareOrderPayload().');
    var orderDetails = {};
    var isReplenishmentOrder = !!(
        mailType != null && mailType == 'Auto Delivery Order Confirmation'
    );

    // Billing Address
    var orderBillingAddressFirstName = order.billingAddress.firstName
        ? order.billingAddress.firstName
        : '';
    var orderBillingAddressLastName = order.billingAddress.lastName
        ? order.billingAddress.lastName
        : '';
    var orderBillingAddressAddress1 = order.billingAddress.address1
        ? order.billingAddress.address1
        : '';
    var orderBillingAddressAddress2 = order.billingAddress.address2
        ? order.billingAddress.address2
        : '';
    var orderBillingAddressCity = order.billingAddress.city
        ? order.billingAddress.city
        : '';
    // var orderBillingAddressPostalCode = (order.billingAddress.postalCode) ? order.billingAddress.postalCode : '';
    var orderBillingAddressStateCode = order.billingAddress.stateCode
        ? order.billingAddress.stateCode
        : '';
    var orderBillingAddressCountryCode = order.billingAddress.countryCode.value
        ? order.billingAddress.countryCode.value
        : '';
    var orderBillingAddressPhone = order.billingAddress.phone
    ...
    if (order.shipments.length > 0) {
        ...
        // Shipping Address
        orderShippingAddressFirstName = order.shipments[0].shippingAddress.firstName
            ? order.shipments[0].shippingAddress.firstName
            : '';
        orderShippingAddressLastName = order.shipments[0].shippingAddress.lastName
            ? order.shipments[0].shippingAddress.lastName
            : '';
        orderShippingAddressAddress1 = order.shipments[0].shippingAddress.address1
            ? order.shipments[0].shippingAddress.address1
            : '';
        orderShippingAddressAddress2 = order.shipments[0].shippingAddress.address2
            ? order.shipments[0].shippingAddress.address2
            : '';
        orderShippingAddressCity = order.shipments[0].shippingAddress.city
            ? order.shipments[0].shippingAddress.city
            : '';
        orderShippingAddressPostalCode = order.shipments[0].shippingAddress
            .postalCode
            ? order.shipments[0].shippingAddress.postalCode
            : '';
        orderShippingAddressStateCode = order.shipments[0].shippingAddress.stateCode
            ? order.shipments[0].shippingAddress.stateCode
            : '';
        orderShippingAddressCountryCode = order.shipments[0].shippingAddress
            .countryCode.value
            ? order.shipments[0].shippingAddress.countryCode.value
            : '';
        orderShippingAddressPhone = order.shipments[0].shippingAddress.phone
        ...
        for (var j in productLineItems) {
            ...
            if (!productLineItem.bonusProductLineItem) {
                priceString = dw.util.StringUtils.formatMoney(
                    dw.value.Money(
                        productLineItem.price.value,
                        session.getCurrency().getCurrencyCode()
                    )
                );
            } else {
                priceString = dw.util.StringUtils.formatMoney(
                    dw.value.Money(0, session.getCurrency().getCurrencyCode())
                );
            }
            ...
            if (productDetail.isVariant()) {
                var variationAttrs =
          productDetail.variationModel.getProductVariationAttributes();
            ...
            productLineItemsArray.push({
                ...
                'Product Image URL'      : imageSize
                    ? productDetail.getImage(imageSize).getAbsURL().toString()
                    : null,
                'Is Sample': isSample
            });
        }
        ...
        if (giftCertificateLineItems && giftCertificateLineItems.length > 0) {
            ...
            var giftCardId =
        dw.system.Site.getCurrent().getCustomPreferenceValue('EgiftProduct-ID');
            var giftCardProductDetail = productMgr.getProduct(giftCardId);
            var giftCardImage = '';
            if (!empty(giftCardProductDetail)) {
                giftCardImage = imageSize
                    ? giftCardProductDetail.getImage(imageSize).getAbsURL().toString()
                    : null;
            }
            for (var l in giftCertificateLineItems) {
                ...
                giftLineItemsArray.push({
                    ...
                    Price             : dw.util.StringUtils.formatMoney(
                        dw.value.Money(
                            giftLineItem.price.value,
                            session.getCurrency().getCurrencyCode()
                        )
                    ),
                    Message : giftLineItem.message,
                    Image   : !empty(giftLineItem.custom.giftCertificateImage)
                        ? giftLineItem.custom.giftCertificateImage
                        : giftCardImage
                });

                items.push(
                    Site.getCurrent().getCustomPreferenceValue('EgiftProduct-ID')
                );
                itemCount += 1;
        ...
        // Order Total
        var merchTotalExclOrderDiscounts =
      order.getAdjustedMerchandizeTotalPrice(false);
        var merchTotalInclOrderDiscounts =
      order.getAdjustedMerchandizeTotalPrice(true);
        ...
        var orderDiscountString = dw.util.StringUtils.formatMoney(
            dw.value.Money(
                orderDiscount.value,
                session.getCurrency().getCurrencyCode()
            )
        ...
        // Sub Total
        var subTotal = merchTotalInclOrderDiscounts.add(
            order.giftCertificateTotalPrice
        );
        var subTotalString = dw.util.StringUtils.formatMoney(
            dw.value.Money(subTotal.value, session.getCurrency().getCurrencyCode())
        );

        // Shipping
        var shippingExclDiscounts = order.shippingTotalPrice;
        var shippingInclDiscounts = order.getAdjustedShippingTotalPrice();
        var shippingDiscount = shippingExclDiscounts.subtract(
            shippingInclDiscounts
        );
        var shippingTotalCost = shippingExclDiscounts.subtract(shippingDiscount);
        var shippingTotalCostString = dw.util.StringUtils.formatMoney(
            dw.value.Money(
                shippingTotalCost.value,
                session.getCurrency().getCurrencyCode()
            )
        );
    ...
    var giftMsg = order.shipments[0].giftMessage
        ? order.shipments[0].giftMessage
        : '';
    orderDetails.GIFT_MESSAGE = giftMsg;

    // Order Details
    // var orderDate = new Date(order.creationDate);
    var orderCreationDate = dw.util.StringUtils.formatCalendar(
        new dw.util.Calendar(new Date(order.creationDate)),
        'yyyy-MM-dd'
    );
    ...
    orderDetails['Item Categories'] =
    require('*/cartridge/scripts/utils/klaviyo/klaviyoUtils').removeDuplicates(
        itemCategories
    );
    orderDetails.$value = orderTotal;
    orderDetails.$event_id = mailType + '-' + order.orderNo;
    orderDetails['Tracking Number'] = order.shipments[0].trackingNumber
