    ...
    try {
        ...
        if (!empty(resultObj)) {
            ...
            if (!empty(response) && response.email === email) {
                ...
                logger.error('checkSubscription() failed for ' + klaviyoUtils.obfuscateKlEmail(email) + '. Error from Klaviyo. Response: ' + resultObj);
                return;
            }
        } else {
            logger.error('checkSubscription() failed for ' + klaviyoUtils.obfuscateKlEmail(email) + '. Error from Klaviyo. Empty Response');
            return;
        }
        return profileId;
    } catch (e) {
        logger.error('checkSubscription() failed for ' + klaviyoUtils.obfuscateKlEmail(email) + '. Error: ' + e.message);
        return;
    }
}
/**
    ...
    try {
        ...
        if (!empty(resultObj)) {
            ...
            if (!empty(response) && response.email === email) {
                ...
                logger.error('subscribeToList() failed for ' + klaviyoUtils.obfuscateKlEmail(email) + '. Error from Klaviyo. Response: ' + resultObj);
                return;
            }
        }
        ...
        logger.error('subscribeToList() failed for ' + klaviyoUtils.obfuscateKlEmail(email) + 'at source ' + source + ' .Error: ' + e.message);
        return;
    }
}
